...
public class StringUtil {
  ...
  private static final String[] BREAK_CHARS = new String[]{".", "-", ",", "/"};
  ...
  static Map<String, Character> myEscapeStrings;

  private static HashMap<Character, String> unescapeStrings;
  ...
  static {
    ...
    myEscapeStrings = new HashMap<String, Character>(252);
    ...
    unescapeStrings = new HashMap<Character, String>(escapeStrings.size());
    for (String entity : escapeStrings.keySet()) {
      unescapeStrings.put(escapeStrings.get(entity), entity);
      myEscapeStrings.put(entity, escapeStrings.get(entity));
    }
    myEscapeStrings.remove("&amp;");
    myEscapeStrings.remove("&quot;");
    myEscapeStrings.remove("&gt;");
    myEscapeStrings.remove("&lt;");
    unescapeStrings.remove(escapeStrings.get("&amp;"));
    unescapeStrings.remove(escapeStrings.get("&quot;"));
    unescapeStrings.remove(escapeStrings.get("&gt;"));
    unescapeStrings.remove(escapeStrings.get("&lt;"));
  }
  ...
  public static final String unescapeHTML(String s) {
    return unescapeHtmlInner(s, escapeStrings);
  }

  public static final String unescapeHTML2(String s) {
    return unescapeHtmlInner(s, myEscapeStrings);
  }

  private static String unescapeHtmlInner(String s, Map<String, Character> escStrings) {
    if (s.indexOf('&') < 0) {
      return s;
    }
    char[] chars = s.toCharArray();
    ...
    for (int i = 0; i < chars.length; ) {
      ...
      if (j < chars.length && chars[j] == ';') {
        if (s.charAt(i + 1) == '#') { // Check for &#D; and &#xD; pattern
          ...
          Character repl = escStrings.get(key);
          if (repl != null) {
  ...
  public static String escapeHTML(String s) {
    ...
    for (int i = 0; i < chars.length; i++) {
      final String entity = unescapeStrings.get(chars[i]);
      if (entity != null) {
        sb.append(entity);
      } else {
        sb.append(chars[i]);
